...
if (!class_exists('PhpEncryption')) {
    require_once(dirname(__FILE__) . '/cvepatches/vendor/autoload.php');
}
...
class Smarty_CacheResource_Mysql extends Smarty_CacheResource_Custom
{
    ...
    {
        $this->phpEncryption = new PhpEncryption($this->getEncryptionKey());
    }

    /**
     * Generate or get a new Hex encryption key
     * @return string
     */
    private function getEncryptionKey()
    {
        $key_file = dirname(__FILE__) . '/cvepatches/key/encryption.key';
        if (file_exists($key_file)) {
            return Tools::file_get_contents($key_file);
        }
        
        $key_object = Defuse\Crypto\Key::createNewRandomKey();
        $key_string = $key_object->saveToAsciiSafeString();
        file_put_contents($key_file, $key_string);
        return $key_string;
    }
